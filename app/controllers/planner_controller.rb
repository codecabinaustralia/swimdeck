class PlannerController < ApplicationController
  def daily_planner
    ...
    @lessons = Lesson.where(lesson_day: @time_brisbane.strftime("%A")).all.paginate(:page => params[:page], :per_page => 300)
    ...
    @task_flags = Task.where(target_user: nil).where(completed: [false, nil]).all
