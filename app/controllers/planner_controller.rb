class PlannerController < ApplicationController
  def daily_planner
  	...
  	@lessons = Lesson.where(start_time: Time.zone.now.beginning_of_day..Time.zone.now.end_of_day).where("start_time < ?", @filter_time.to_date + 1.day).all.order(:start_time).paginate(:page => params[:page], :per_page => 30)
